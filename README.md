...
> **Note:** Unlike `query_similar_proteins.py`, the accepted chain-length range and the Cα distance cutoff are **hard-coded** in `create_v2_proteograms.py` — `sequence_len_lower_cutoff=20`, `sequence_len_upper_cutoff=200`, and `calpha_atom_distance_cutoff=10` (Å) — not exposed via `config.yml` or CLI flags. Chains outside the [20, 200] residue range are skipped. To batch-create proteograms for larger or smaller proteins, edit these values directly in [create_v2_proteograms.py](scripts/v2/create_v2_proteograms.py#L170-L172).
