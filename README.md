...
> **Note:** The `--annot_file` label file is `scope_eval_set_labels.tsv`, which is generated by `evaluate_methods_v2.py` ([Step 6](#step-6--evaluate-all-methods)) — run that at least once to produce it. It has columns `pdb_id_chain`, `proteogram_file`, `class`, `fold`, `superfamily`, `family`. This is **not** the annotation lookup produced by `create_scope_annotation_file.py` (which holds GO terms and has different columns); that file will not work here.
